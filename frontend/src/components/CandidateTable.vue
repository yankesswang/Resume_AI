...
const headers = [
  ...
  { title: '大學', key: 'university', sortable: false, width: '200px' },
  { title: '碩士', key: 'masters', sortable: false, width: '200px' },
  { title: '年資', key: 'years_of_experience', sortable: true, width: '100px' },
...
const TOP_UNIVERSITY_KEYWORDS = ['台灣大學', '臺灣大學', '台大', '清華大學', '清大', '交通大學', '交大', '陽明交通大學', '成功大學', '成大', '政治大學', '政大']
...
const filteredCandidates = computed(() => {
  return props.candidates.filter((c) => {
    if (filters.searchName) {
      ...
      if (!c.name || !c.name.toLowerCase().includes(query)) return false
    }
    ...
    if (filters.scoreRange) {
      if (!matchScoreRange(c.overall_score, filters.scoreRange)) return false
    }
